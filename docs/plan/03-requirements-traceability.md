...
These rows were reconciled against the shared continuation tree on 13 August. Paths are relative to
the `dl-finance-workbench` repo; `kit:` means demo-kit, reached through `vendor/demo-kit/`. A path is
evidence only when it can be opened and its stated check exists. Hosted-release claims are marked
pending because GitHub Actions and deployment were not part of the continuation.
...
| FW-ANALYSIS-001 | Compare actual with budget and/or forecast; identify material variances; reproducible and traceable | `packages/analysis/src/bridge.ts` · `packages/measures/src/materiality.ts` · `web/app/app/performance` | Bridge tests assert every contribution sums to the movement; finding evidence names the policy and threshold; Explore opens the same governed measure to source rows |
...
| FW-AI-005 | Commentary for month, quarter, half-year and year, with like-for-like comparators, stating the period and comparator used | `packages/model/src/period.ts` (`HALF_YEAR`) · `packages/measures/src/comparator.ts` · Commentary | The period spine and five comparator semantics are implemented and every commentary identity prints its period/comparator. **Partial:** the seeded approval queue currently demonstrates monthly items; quarter, half-year and year commentary records are not yet seeded on the surface |
...
| FW-DASH-002 | Each board item drills into the KPI movement, driver analysis, detailed commentary and source lineage | `packages/analysis/src/detectors.ts` typed actions · `web/lib/commentary.ts` · `packages/analysis/src/pivot.ts` | Detector tests parse each typed route and required state; Commentary resolves movement → drivers → accounts → source rows; Explore's group drill ties exactly and every row names a vintage |
...
| A future deployed build must be the build that was verified | `kit: packages/shell/src/health.ts` + `.github/workflows/deploy.yml` | **Pending:** after an authorised deploy, `curl /api/health \| jq .commit` must equal the deployed SHA. GitHub Actions and deployment were not run in the continuation |
| The replacement deck's screenshots come from the running app | `web/deck.config.mjs` · `web/public/deck.html` · `docs/deck/demo.pdf` | All ten product shots were regenerated from the current app; origin lint, aspect sync and all twelve slide renders pass without overflow; the committed tagged PDF has twelve pages and was visually checked from rendered pages |
