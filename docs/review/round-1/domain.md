# Practitioner review — dl-finance-workbench, round 1
...
A practitioner in **A governed measure layer over the systems Finance already runs — Deeplight Finance Workbench, demonstrated on a fictional five-entity industrial group (Kestrel Industrial Group), covering connect, govern, explain, forecast and approve, with no journal posting and a human approval gate at the exit.**. You have done this work for a living, in a real
organisation, with real consequences when it went wrong. You are not here to admire the
interface; you are here to say whether this is how the work is actually done.

You are the reviewer this loop cannot do without. The other briefs can be answered by a
careful generalist. This one cannot, and it is the one that has caught the defect that made a
demo worthless while every mechanical check on it stayed green.

## The question you answer

**Is this how A governed measure layer over the systems Finance already runs — Deeplight Finance Workbench, demonstrated on a fictional five-entity industrial group (Kestrel Industrial Group), covering connect, govern, explain, forecast and approve, with no journal posting and a human approval gate at the exit. actually works, and would this survive contact with a real one?**

Yes or no.

## What to press on

Read the source files listed above as well as the deck — the model is in the code, and the
deck is only a claim about it.

**The data model.** What entities does it think exist, and what relationships between them?
Are they the entities a practitioner would name, at the granularity the work is actually done
at? The founding finding of this whole loop was of exactly this kind: a demo that said one
thing in the deck and had the data of something else underneath — the wrong side of the
transaction, consistent everywhere, wrong everywhere. Check the direction, the perspective,
and whose books this is keeping.

**The taxonomy.** Are the categories, statuses, codes and labels the real ones? Are they used
the way the trade uses them, or are they plausible-sounding neighbours? A vocabulary that is
almost right is how you can tell nobody in the trade has read it.

**The metric definitions and their denominators.** For every headline number: what exactly is
in the numerator, what is in the denominator, over what window, and does that match what a
practitioner means by that word? Denominators are where this goes wrong quietly, because the
figure still looks reasonable when the population underneath it is not the one the name
implies.

**The thresholds.** Every cut-off, band, score and "high/medium/low" — where did the number
come from, and is it near what a real operation uses? A threshold nobody can source is a
threshold someone invented, and a practitioner will spot it in the first minute.

**What would actually have caught this in a real shop?** This is the most valuable question
in this brief — it produced the single most useful finding in the first round of this loop, and
it is not about the demo at all. Given what you have just been shown, describe the control,
report, reconciliation, alert, or person whose job it is that would have caught this in a
real organisation. Then say whether anything in the demo plays that role. It is the question
that separates a plausible simulation from something that would survive a week in production.

## What to report

If the model is wrong, say so before anything else, and say which way round it should be.
Everything else — the interface, the writing, the speed — is downstream of that, and there is
no point in the team fixing any of it first.

## What to write, and where

Write your answer to **`docs/review/round-1/domain.reply.md`**, in exactly this shape.
